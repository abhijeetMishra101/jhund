...
import { BOT_ROLES } from '@/lib/templates/seed'
...
describe('bot system prompts — propose_github_action tool contract', () => {
  it.each(GITHUB_CAPABLE_ROLES)(
    ...
    (roleKey) => {
      const bot = BOT_ROLES.find((b) => b.role_key === roleKey)
      expect(bot, `No bot found with role_key "${roleKey}"`).toBeDefined()
      expect(bot!.system_prompt).toContain('propose_github_action')
    }
  )

  it('ops bot does not perform GitHub actions (no tool instruction needed)', () => {
    const ops = BOT_ROLES.find((b) => b.role_key === 'ops')
  ...
  it.each(GITHUB_CAPABLE_ROLES)(
    ...
    (roleKey) => {
      ...
      expect(bot.system_prompt).not.toMatch(/post a plain-English plan/i)
      expect(bot.system_prompt).not.toMatch(/propose a plan(?! using| card)/i)
    }
  )
})
